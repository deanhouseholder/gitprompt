...
menu() {
  ...
  main_menu() {
    ...
    local default_highlight_bg="\e[48;5;4m"  # Blue background
    # Define keyboard/mouse escape codes
    local key_esc=$'\x1B'
    ...
    local key_mouse_scroll_up=$'\e[<64;'
    local key_mouse_scroll_down=$'\e[<65;'
    ...
    # Initialize screen and enable mouse tracking
    tput civis >&2
    printf "\033[?1000h" >&2  # Enable mouse tracking
    printf "\033[?1006h" >&2  # Enable SGR extended mouse coordinates
    ...
    # Read in a key with any modifiers
    read_keypress() {
      local keypress byte
      keypress=""
    ...
    # Main loop to draw menu, handle input, and hand-off selection
    while true; do
      ...
      case "$input" in
        ...
        "$key_esc"|'q'|'Q')
          ...
          printf "\033[?1000l" >&2  # Disable mouse tracking
          printf "\033[?1006l" >&2  # Disable SGR extended mouse coordinates
          tput cnorm >&2
          return 255
        ;;

        # Move selection up by one item
        "$key_up1"|"$key_up2"|"$key_left1"|"$key_left2"|"$key_mouse_scroll_up"*)
          ((selected--))
          if (( selected < 0 )); then
        ...
        "$key_down1"|"$key_down2"|"$key_right1"|"$key_right2"|"$key_mouse_scroll_down"*)
          ((selected++))
